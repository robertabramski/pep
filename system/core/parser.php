	...
	class Parser
	{
		...
		public function parse($text, $data = array(), $callback = false, $allow_php = false)
		{
			...
			else
			{
				...
				$data = array_merge(Parser::$data, $data);
			...
			// set $cumulative_noparse to true and use Parser::inject_noparse($text); immediately
		...
		public function parse_callback_tags($text, $data, $callback)
		{
			...
			while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE))
			{
				...
				{
					$cb_data = $data;
					if ( !empty(Parser::$callback_data))
					{
						$cb_data = array_merge(Parser::$callback_data, $data);
					}
		...
		public function parse_recursives($text, $orig_text, $callback)
		{
			...
			if (preg_match($this->recursive_regex, $text, $match))
			{
				...
				$tag = $match[0];
				$next_tag = null;
				$children = Parser::$callback_data[$array_key];
		...
		{
			if (isset(Parser::$extractions['noparse']))
			{
				foreach (Parser::$extractions['noparse'] AS $hash => $replacement)
				{
		...
		protected function create_extraction($type, $extraction, $replacement, $text)
		{
			...
			Parser::$extractions[$type][$hash] = $replacement;
		...
		protected function inject_extractions($text, $type = null)
		{
			if ($type === null)
			{
				foreach (Parser::$extractions as $type => $extractions)
				{
					foreach ($extractions as $hash => $replacement)
					{
						if (strpos($text, "{$type}_{$hash}") !== false)
						{
							...
							unset(Parser::$extractions[$type][$hash]);
						}
					}
				}
			}
			...
			{
				if ( ! isset(Parser::$extractions[$type]))
				{
					return $text;
				}
	
				foreach (Parser::$extractions[$type] as $hash => $replacement)
				{
					if (strpos($text, "{$type}_{$hash}") !== false)
					{
						$text = str_replace("{$type}_{$hash}", $replacement, $text);
